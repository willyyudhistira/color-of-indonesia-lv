...
@section('content')
    ...
    <div class="relative px-20 py-20"> {{-- 2. Event Mendatang Section --}}
        ...
        @if($upcomingEvents->isNotEmpty())
            {{-- Menggunakan Swiper.js untuk menampilkan semua event mendatang --}}
            <div class="max-w-5xl mx-auto mb-20">
                <swiper-container 
                    slides-per-view="1" 
                    navigation="true" 
                    autoplay-delay="5000" 
                    loop="{{ $upcomingEvents->count() > 1 ? 'true' : 'false' }}" {{-- Loop hanya aktif jika ada lebih dari 1 slide --}}
                    class="rounded-xl shadow-2xl">

                    @foreach($upcomingEvents as $event)
                    <swiper-slide>
                        <div class="bg-white overflow-hidden md:flex group rounded-xl shadow-lg">
                            <div class="md:w-1/2">
        ...
        @else
            {{-- Placeholder jika tidak ada event mendatang --}}
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @forelse ($events as $event)
                <div class="bg-white rounded-xl shadow-lg overflow-hidden group">
                    <div class="overflow-hidden h-56"><img src="{{ asset('storage/' . $event->hero_image_url) }}"
                    ...
                    <div class="p-6">
                        ...
                        <a href="{{ route('events.show', ['slug' => $event->slug]) }}"
                            class="inline-block bg-purple-100 text-purple-700 font-bold py-2 px-5 rounded-full hover:bg-purple-200 transition-colors text-sm">More
                            Information</a>
                    </div>
                </div>
            @empty
